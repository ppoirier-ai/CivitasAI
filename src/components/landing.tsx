...
import {
  ...
  FileText, ShieldCheck, ShoppingBag, ArrowRight, BookOpen, Infinity as InfinityIcon, Target,
} from 'lucide-react';
    ...
    <div className="-mx-4 sm:-mx-6 lg:-mx-8 overflow-hidden">
      ...
      <section className="max-w-6xl mx-auto px-6 py-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {loading ? (
            [0, 1, 2].map((i) => (
              <div key={i} className="aspect-[3/4] rounded-xl bg-gray-900/40 border border-gray-800/40 animate-pulse" />
            ))
          ) : featured.length === 0 ? (
      ...
      {/* ============ WHAT'S INSIDE ============ */}
      <section id="inside" className="relative py-20 border-t border-gray-800/40 scroll-mt-20">
        ...
        <div className="relative max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {SECTIONS.map(({ icon: Icon, title, desc }) => (
              <div
                key={title}
                className="rounded-xl border border-gray-800/50 bg-gray-900/40 p-5 hover:border-[#2EC4C6]/30 hover:bg-gray-900/60 transition-all"
              >
      ...
      <section className="relative py-24 overflow-hidden border-t border-gray-800/40">
        ...
        <div className="relative max-w-2xl mx-auto text-center px-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mt-8">
            <Link href="/opportunities">
              <Button className="bg-[#2EC4C6] hover:bg-[#28B0B2] text-black font-semibold h-12 px-8 rounded-xl text-sm transition-all active:scale-[0.98]">
                <Rocket className="w-4 h-4 mr-2" /> Explore the Marketplace
              </Button>
            </Link>
            <Link href="/auth/login">
              <Button variant="outline" className="border-gray-700/60 text-gray-300 hover:text-white hover:border-[#2EC4C6]/50 h-12 px-8 rounded-xl text-sm">
